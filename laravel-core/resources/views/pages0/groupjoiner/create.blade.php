...
@section('subtitle', "Create a joiner")
@section('content')
@php
$user = Auth::user();
@endphp
<form method="POST" action="{{route('group_joiner_create')}}" enctype="multipart/form-data">
  @csrf
  <div class="row">
    <div class="col-12 col-lg-12 col-xxl-12 d-flex">
      <div class="card flex-fill">
        <div class="card-header d-flex justify-content-between align-items-center">
          <h5 class="card-title mb-0">Create a joiner</h5>
      ...
      <div class="card flex-fill">
        ...
        <div class="card-body">
          ...
          <div class="mb-3">
            ...
            <input type="text" name="keywords" id="keywords" value="{{old('keywords')}}" class="form-control">
            @error('keywords')
              <div class="text-danger">{{ $message }}</div>
            @enderror
          </div>
        ...
        <div class="card-body">
          <div class="row">
            <div class="mb-3">
              ...
              <input type="number" name="join" value="{{old('join')}}" id="join" class="form-control">
              @error('join')
                <div class="text-danger">{{ $message }}</div>
              @enderror
            </div>
            <div class="mb-3 col-md-6">
              ...
              <input type="number" name="each" value="{{old('each')}}" id="each" class="form-control">
              @error('each')
                <div class="text-danger">{{ $message }}</div>
              @enderror
            </div>
            ...
            <div class="mb-3 col-md-6">
              ...
              <input type="text" name="start_at"  data-inputmask-regex="^(?:[01][0-9]|2[0-3]):[0-5][0-9]$" required value="{{old('start_at')}}" id="start_at" class="form-control">
              @error('start_at')
                <div class="text-danger">{{ $message }}</div>
              @enderror
            </div>
            <div class="mb-3 col-md-6">
              ...
              <input type="text" name="end_at"  data-inputmask-regex="^(?:[01][0-9]|2[0-3]):[0-5][0-9]$" required value="{{old('end_at')}}" id="end_at" class="form-control">
              @error('end_at')
                <div class="text-danger">{{ $message }}</div>
              @enderror
            </div>
            <div class="mb-3">
              ...
              <input type="number" name="max_join" value="{{old('max_join')}}" id="max_join" class="form-control">
              @error('max_join')
                <div class="text-danger">{{ $message }}</div>
              @enderror
            </div>
